...
<div class="navbar">
    ...
    <div class="reserva-container">
        ...
        <div class="imagem-container">
            <img id="quarto-imagem" src="" alt="Imagem do Quarto">
        </div>
        ...
        <h1>Faça sua Reserva</h1>
            ...
            <option value="selecione">Selecione...</option>
                <option value="domo">Quarto Domo</option>
                <option value="charrua">Quarto Charrua (Bus)</option>
                <option value="suite">Suíte com Cozinha</option>
                <option value="chale">Chalé Família</option>
                <option value="cabana">Cabana</option>
                <option value="estacionamento">Estacionamento para Overlanders</option>
            </select>
            ...
            <input type="number" id="adultos" name="adultos" min="1" max="10" placeholder="Adultos" required>

            <div style="display: flex; align-items: center;">
            <label for="criancas" style="margin-right: 10px; margin-top: 3px;">Incluir Crianças?</label>
            <input type="checkbox" id="criancas" name="criancas" value="sim" style="transform: scale(1.3); margin-right: 5px;">
            <label for="criancas" style="margin-right: 15px; margin-top: 4px;">Sim</label>
        </div>
            <br>

            <input type="number" id="numero_criancas" name="numero_criancas" min="0" max="10" placeholder="Número de Crianças" disabled>
    ...
    <script>
        document.getElementById('quarto').addEventListener('change', function() {
            const imagem = document.getElementById('quarto-imagem');
            const escolha = this.value;
            const imagens = {
                'domo': 'imagens/quarto_domo.jpeg',
                'charrua': 'imagens/quarto_charrua.jpeg',
                'suite': 'imagens/quarto_suitecozinha.jpg',
                'chale': 'imagens/quarto_chale.jpg',
                'cabana': 'imagens/quarto_cabana.jpg',
                'estacionamento': 'imagens/estacionamento.jpg'
            };

             if (escolha in imagens) {
            imagem.src = imagens[escolha];
            imagem.style.display = 'block';
            document.querySelector('.form-image-wrapper').classList.add('slide');
        } else {
            imagem.style.display = 'none';
            document.querySelector('.form-image-wrapper').classList.remove('slide');
        }
    });

    document.getElementById('criancas').addEventListener('change', function() {
        const numeroCriancas = document.getElementById('numero_criancas');
        if(this.checked) {
            numeroCriancas.disabled = false;
        } else {
            numeroCriancas.disabled = true;
            numeroCriancas.value = "";
        }
    });
    </script>
