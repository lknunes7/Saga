...
<div class="navbar">
    ...
    <script>
        function confirmarExclusao(Codigo, Nome) {
            if (window.confirm("Deseja realmente apagar o registro:\n" + Codigo + " - " + Nome)) {
                window.location = "excluirUsuario.php?id=" + Codigo;
            }
            return false;
        }

        function confirmarEdicao(Codigo, Nome) {

            window.location = "editarUsuario.php?id=" + Codigo;
    ...
    $id = $_SESSION['id'];
    $SQL = "SELECT Codigo, Nome, Descricao, DataNascimento, Telefone FROM usuario WHERE Codigo = $id";

    // Executa a consulta
    $r = $conn->query($SQL);
    
    // Verifica se a consulta retornou algum resultado
    if ($r && $dados_usuario = $r->fetch_assoc()) {
        $_SESSION['id'] = $dados_usuario['Codigo'];
    } else {
        // Caso não encontre o usuário, redireciona ou exibe uma mensagem
    ...
    <input name="txtNome" id="txtNome" type="text" value="<?php echo htmlspecialchars($dados_usuario['Nome']); ?>" disabled><br><br>

    <label for="">Descrição:</label>
    <input name="txtDesc" id="txtDesc" type="text" value="<?php echo htmlspecialchars($dados_usuario['Descricao']); ?>" disabled><br><br>

    <label for="">Data de Nascimento:</label>
    <input name="txtDataNascimento" id="txtDataNascimento" type="text" value="<?php echo htmlspecialchars($dados_usuario['DataNascimento']); ?>" disabled><br><br>

    <label for="">Telefone:</label>
    <input name="txtTelefone" id="txtTelefone" type="text" value="<?php echo htmlspecialchars($dados_usuario['Telefone']); ?>" disabled><br>

    <div id="container">

        <div id="divleft">
            <button style="width: 175px; height: 30px;" class="btnExcluir" onclick="return confirmarExclusao('<?php echo $_SESSION['id']; ?>', '<?php echo htmlspecialchars($dados_usuario['Nome']); ?>')">Excluir</button>
        </div>

        <div id="divright">
            <button class="btnEditar" onclick="return confirmarEdicao('<?php echo $_SESSION['id']; ?>', '<?php echo htmlspecialchars($dados_usuario['Nome']); ?>')">Editar</button>
        </div>
